...
\title{Dynamic and Adaptive Maintenance Policies\\ for Complex Systems\\ based on Real-Time Data}
...
\item<2-> for each component type:
 ...
 \end{itemize}
\item<3-> cost parameters %, preparation time %\raisebox{0.5ex}{\tikz{\draw[<->, very thick] (0,0) -- (0.7,0);}}
\end{itemize}
\uncover<4->{%
Input during run-time (preferred automatic):}
\begin{itemize}
...
\end{scope}}
\uncover<2->{%
\draw[decorate,decoration=brace,very thick] (2.9,1.6) -- (2.9,0.4) node[midway, right=0.25cm] {\includegraphics[width=1.5cm]{compArel0}};}
\end{tikzpicture}
\end{frame}
...
\end{align*}}};
\draw[decorate,decoration=brace,line width=0.8mm] (0.75,-3.1)  -- (-1.7,-3.1) node[cyanrand, below=0.5cm] {\parbox{0.51\textwidth}{\centering%
survival signature\\
$= P(\text{system functions} \mid \{l_k \text{ \comp{k}'s function}\}^{1:K})$
}};
\draw[decorate,decoration=brace,line width=0.8mm] (5.7,-3.1)  -- (1.6,-3.1) node[cyanrand, midway, below=0.5cm] {\parbox{0.3\textwidth}{%
Probability that $l_k$ of the $\cknow$ \comp{k}'s function
}};
\end{tikzpicture}
\end{frame}

\begin{frame}{\large Expected One-cyle Unit Cost Rate}
\begin{itemize}
...
\item[$c_p$] cost of planned maintenance action
\item[$c_u$] cost of unplanned maintenance action
\end{itemize}
\begin{align*}
g(\tau \mid \Tsysnow = t) &=
\begin{cases}
c_p / \tau  & \text{if } t \ge \tau \qquad\text{ (cost rate if failure after $\tau$)}\\
c_u / t     & \text{if } t < \tau   \qquad\text{ (cost rate if failure before $\tau$)}
\end{cases} \\
