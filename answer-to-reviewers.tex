...
We now also emphasize in the conclusion that the proposed method should be seen as a first step towards
...
is reflected in a loss of current (detectable from the control centre),
but proper health monitoring is cumbersome and costly.
***}
...
In the re-write of Section~5, we tried to emphasize the pilot character of our approach,
...
\emph{We are sorry that we defined RLD too late, and now define it at the first use.}
...
So the actual question is rather whether inverse Gamma distributions can adequately reflect the expert's opinions.
...
leading to the posterior predictive approaching the Weibull with $\lambda_k$ equal to $y_k$.
