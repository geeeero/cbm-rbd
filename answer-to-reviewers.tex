...
and for such a system, indeed a simpler methodology like the keystone method could very well suffice.
However, our survival signature based method is also applicable for larger systems where the keystone method
is not applicable or cumbersome.
Furthermore, the \textsf{R} package \texttt{ReliabilityTheory} allows to easily calculate the
survival signature from a given system graph.\\
...
but I would think more repetitions (say, 50 or 100 instead of 20) would be better.
...
\emph{***??? It seems we have confused the reviewer thoroughly.
We need to clarify that we do not use a \textbf{usual} degradation signal, like vibration measurements.
But we do get a (time-) continuous signal from the components of the system, telling us whether they work or not.
...
\emph{***We defined $\Rsys(t) := P(\Tsys > t)$ on p.~5 above Equation~1,
...
\emph{We had placed Figure~2 (depicting the flowchat for the policy, now Figure~3) directly before Section~5.1 which explains the policy,
...
\emph{We agree with Reviewer~2 that such a sketch is helpful, and provide it now as Figure~2.}
...
\emph{We agree that the assumption of a fixed known shape parameter is a serious limitation of our approach. 
...
We now also emphasize in the introduction and the conclusion that the proposed method should be seen as a first step towards
a completely new class of CBM policies.***}
...
\emph{***Agreed, this is another important simplifying assumption that we needed to make in this manuscript,
...
\emph{***depends on the scale of failure times? seconds / minutes? ??? not sure what is meant by the second sentence.}
