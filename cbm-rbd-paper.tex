...
[node distance=10mm,
 font=\footnotesize,
 box/.style={draw, rectangle, thick, minimum height=6mm, minimum width=26mm, rounded corners=2mm},
...
\node (tauchck) [box, below=6mm of sysfail] {$\tausnow < \tthresh$?};
\node (gotonxt) [box, below=6mm of tauchck] {\parbox{15ex}{\centering continue to\\ next $\tnow$}};
\draw [thick, dashed, rounded corners=2mm] ($(gotonxt.south east) + (2mm,-1mm)$) rectangle ($(sysfail.north west) + (-5mm,1mm)$) node[above left=0mm and -14mm] {each $\tnow$};
\node [above left=0mm and -8mm of startup] {$t=0$};
\node (correct) [box, right=of sysfail, minimum width=18mm] {\parbox{14ex}{\centering corrective\\ maintenance\\ (cost $c_u$)}};
...
Figure~\ref{fig:tauhist1fig2} illustrates these relations in the scenario depicted in Figure~\ref{fig:ghist1fig2T}.
...
***$\tausnow$ (left panel) first decreases, then starts to increase during failure-free periods,
...
***move some text from figure~\ref{fig:tauhist1fig2T} caption to here?***
