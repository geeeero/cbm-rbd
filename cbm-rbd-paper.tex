...
It is also possible that $\gnow(\tau)$ is monotonely decreasing.
...
Both factors influence $\Rsysnow(t)$ and thus $\tausnow$.
...
As discussed in the Introduction and above,
...
and worthy for a seperate contribution.
...
***we calculate $\Rsysnow(t)$ on discrete grid until some horizon, optimize $\tausnow$ over same discrete grid.
...
***if min $\gnow(\tau)$ at horizon, corrective maintenance is optimal: policy switch!
...
use simple system layout, example history of compnents failing

use the simplified brake system from ASCE-ASME paper?
