...
and we will now show how the system reliability function, given component models, can be efficiently calculated using the survival signature.
...
takes  the value 1 if the system functions and 0 if the system does not function for state vector $\vec{x}$ \cite{BP75}.
...
\section{Component posterior predictive distributions***}
\label{sec:postpred}

To calculate the current system reliability curve,
we thus need the probabilities $P(C^k_t = l_k)$,
which give the number of components that function at times $t > \tnow$.
In our Bayesian framework, these probabilities are given
by a so-called predictive distribution that can be derived from the distribution over $\lambda$.
This distribution needs to account for expert knowledge and test data;
following our comment at the end of Section~\ref{sec:weibull},
we assume here that ***change notation to $n_k^{(\text{test})}$ and $n_k^{(\text{now})}$???

but also for the information gained from the current run of the system:
both failure times of components (if any)
and the right-censoring time $\tnow$ for each non-failed component in the system
can be used to update $\nkz$ and $\ykz$.

In analogue to the notation from Section~\ref{sec:weibull},
let $n_k = e_k^{\tnow} + c_k^{\tnow}$ be the number of type $k$ components in the monitored system,
where $e_k^{\tnow}$ is the number of components that have failed by $\tnow$,
and $c_k^{\tnow}$ is the number of components that have not failed by $\tnow$.
We can thus collect these observations, as of $\tnow$, in a vector
$\vec{t}_k^{\tnow} = (t_{k,1}, \ldots, t_{k,e_k}, \tpnow, \ldots, \tpnow)$,
containing $n_c$ right-censored obsevations $\tpnow$.
