...
how this method can be used to find the system reliability function $\Rsys(t)$ at current time $\tnow$ for a monitored system,
...
\subsection{Component posterior predictive distributions}
\label{sec:postpred}
...
Denote by $\nkn$ and $\ykn$ the parameters obtained
by updating the prior parameters $\nkz$ and $\ykz$ using test data.
The corresponding distribution $f(\lambda_l \mid \nkn, \ykn)$
thus accounts for both expert knowledge and test data.
However, following our comments at the end of Section~\ref{sec:weibull},
...
containing $c_k^{\tnow}$ right-censored observations $\tpnow$.
