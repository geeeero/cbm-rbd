...
like in \citet{2013:si-et-al}, we cannot use the renewal-reward theory based calculation method,
...
\end{itemize} 
***As an example, Consider the case $l_M = 0, l_H = 1, l_C = 0, l_P = 1$,
...
As the type $P$ components are exchangeable, each has the same probability of being the functioning component,
and so the probability of $P_3$ or $P_4$ being the functioning component is $0.5$, and $\Phi(0, 1, 0, 1) = 0.5$.

The \textsf{R} package \texttt{ReliabilityTheory} \citep{2016:aslett-RT}
provides a convenient function to calculate the survival signature based on a given graph.
...
\centering
\begin{tikzpicture}
\node {exemplary timeline};
\end{tikzpicture}
\caption{***exemplary timeline sketch}
\label{fig:timeline}
\end{figure}
