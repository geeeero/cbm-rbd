...
Replacing not all components during maintenance
...
(Figures~\ref{fig:ghist1fig2T} and \ref{fig:tauhist1fig2T}).
...
Judging the adequacy of the prior parameter choices thus requires the sampling model to be correct.
Methods for checking the sampling model are discussed, e.g., in \citet{2000:bayarri-berger}.
Figure~\ref{fig:compprior1fig1} indicates that, e.g., the median prior expected failure times are
...
leading to higher expected operational cycle cost rates $\gstarnow$ (right panel).
...
We thus can compare policies that use information gained in previous operational cycles
...
We also distinguish condition-based maintenance policies with and without continuous parameter update
...
%Here, most condition-based and all age-based policies cannot avoid system failures entirely.
Here, only CBM-cpu can avoid system failures entirely.
Both CBM-epu and CBM-npu result in one system failure in the $20 \times 5 = 100$ system runs,
whereas the two age-based policies each result in ten system failures.
...
managing to avoid system failures entirely. 
Still, CBM-npu does surprisingly well, considering that it never updates prior component parameters. 
...
Overall, it seems that CBM-cpu can exert its adaptive strengths most effectively
...
There are several aspects in our modeling that warrant further investigation.
...
by determining the optimal moment for the next inspection of the system.
...
the proposed policy could be extended to accommodate common cause failures.
...
leading to sets of residual life distributions $\Rsysnow(t)$.
Consequently, we would obtain a set of expected operational cycle cost rate functions $\gnow(\tau)$.
%and this would lead to sets of expected operational cycle cost rate functions $\gnow(\tau)$.
