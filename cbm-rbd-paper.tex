...
However, \cite{2016:walter-coolen} did not consider any maintenance policies,
...
so we denote the current survival signature by $\Phinow(l_1,\ldots,l_K)$.
...
Define the set of evaluation time points as $\{ \tnow : \tnow = m \delta, m \in \naturals_0\}$,
%where $m = 0, 1, 2, \ldots$,
...
as soon as $\tausnow$ decreases below $\delta$.
...
This criterion, as detailed in Section~\ref{sec:costrate} below,
...
[node distance=10mm,
 font=\footnotesize,
 box/.style={draw, rectangle, thick, minimum height=6mm, minimum width=28mm, rounded corners=2mm},
...
\end{tikzpicture}
\caption{Operational procedure under the proposed maintenance policy,
showing a single operational cycle,
including the loop back to system startup that starts a new cycle.}
\label{fig:procedure}
\end{figure}
...
when evaluation of the RLD is fast.
...
A preventive maintenance action is triggered at the smallest $m \in \naturals_0$
such that $\tausnow < \delta$ for $\tnow = m \delta$.
When $\tausnow$ falls below $\delta$,
the optimal moment of maintenance is so close to present time $\tnow$
that in continuing to the next $\tnow = (m+1)\delta$, one exceeds the optimal system failure risk
as determined by the trade-off made by $\gnow(\cdot)$.
This means that a preventive maintenance action should be carried out at $\tnow = m\delta$, which incurs cost $c_p$.
When the system fails, instead a corrective maintenance action is carried out, incurring cost $c_c$.
The current cycle ends when either preventive or corrective maintenance is carried out,
and a new cycle starts with a system with all components (as good as) new.
%choice of a failure threshold in usual CBM policies.
%This failure threshold gives the level of the degradation signal where 
...
CBM is known to be most effective when the ratio $c_p / c_c$ is small
(\textbf{***cite de Jonge and Teunter working paper on relative benefit of CBM}).
%
As discussed above,
we assume that both $c_p$ and $c_c$ include the cost of replacing all components in the system,
...
c_u /  \tsysnow      & \text{if }        \tnow < \tsysnow  <  \tnow + \tau \,.
\end{cases}
\label{eq:gtau}
\end{align}
...
Taking the expectation over $\Tsysnow$ in \eqref{eq:gtau} leads to the expected unit cost rate
\begin{align}
\gnow(\tau) &= \E[g(\tau \mid \Tsysnow)] \nonumber\\ 
            &= \frac{c_p}{\tnow + \tau} \Rsysnow(\tnow + \tau)
              + c_u \int_0^\tau \frac{1}{\tnow + \theta} \fsysnow(\tnow + \theta) \dd \theta\,.
\label{eq:gtnowtau}
\end{align}
Thus $\gnow(\tau)$ gives the unit cost rate that we expect to incur over the complete operational cycle
...
Finally, $\tausnow := \arg\min \gnow(\tau)$
is the cost-optimal moment of maintenance as of $\tnow$,
...
The minimal expected unit cost rate $\gstarnow := \gnow(\tausnow)$ corresponding to $\tausnow$
thus is, as of $\tnow$, the lowest unit cost rate for the current operational cycle
...
Figure~\ref{fig:tauhist1fig2T} illustrates these relations in the scenario depicted in Figure~\ref{fig:ghist1fig2T}.

As can be seen there and in the numerical examples in Section~\ref{sec:examples} below,
...
Common cause failures will seriously influence the RLD and thus $\tausnow$,
and we will comment on possible extensions in the Outlook (Section~\ref{sec:outlook}).
...
$\tausnow$ (top panel) decrease weakens and attains a near-constant speed from about $\tnow = 2$ during failure-free periods,
with discrete drops at the failures of components $P_2$, $P_3$ and $H$ only.
%With the failure of $H$ at time $\tnow = 8$,
...
Consequently, $\gstarnow$ (bottom panel) is steadily decreasing,
...
At $\tnow = 6$, the expected unit cost rate is $0.033$,
and so considerably lower than the expected cost rate of $0.2 / 1.6 = 0.125$
for an age-based policy not accounting for component information.}
...
We now elucidate in more detail the case study used for the previous illustrative figures.
...
***move some text from figure~\ref{fig:tauhist1fig2T} caption to here or repeat?***
...
leading to lower expected unit cost rates $\gstarnow$ (right panel).
$\tausnow$ for the non-adaptive algorithm drops below $\delta = 0.1$ at $\tnow = 4$,
while maintenance is triggered for the adaptive algorithm at $\tnow = 6$.
...
The effect of this continuous parameter update is illustrated in Figure~\ref{fig:tauhist1fig3T}.
...
***$\tausnow$ values for the model with parameter updates vary around those obtained without parameter updates,
same for $\tstarnow$ and $\gstarnow$.
With $\delta = 0.1$, the moment of maintenance is $2.6$ and $2.5$, respectively,
this shows that for this failure history, where there is indeed no need to update,
and $\tausnow$ behaviour is dominated by the update of the system reliability block diagram.
...
***without update $\tausnow$ consistently higher,
adaptive algorithm takes surprisingly early failures into account.
Unlike in the other two scenarios, the overall trend for $\tstarnow$ is a decrease,
expected cycle costs are underestimated without update***

Model with update consistently suggests to maintain earlier,
but for both models, $\tausnow$ drops below $\delta = 0.1$ upon the failure of H at $\tnow = 1$.
Like in failure history 2, the structural information plays an important part in the policy.
This aspect will be confirmed in the small simulation study described next.
...
$\esys$ is the number of system failures,
$\mrsys$ is the mean realized system runtime (I called this 'cycle length' before),
$\bar{g}$ is the mean realized unit cost rate.

CBM = condition-based maintenance
ABM = age-based maintenance
CM = corrective maintenance
cpu = continuous parameter update
epu = end of cycle parameter update
npu = no parameter update



Case 1: failure behaviour as expected,
failure times simulated using Weibull scale parameters $\lambda_k = \ykz$ from Table~\ref{tab:priorparams}.

Obviously, CM has always 5 system failures in the 5 cycles, while CBM and ABM (mostly) avoid system failures. Mean system runtimes are longest for CM because it runs the system until failure, but this leads to the highest mean unit cost rates. CBM with continuous parameter update has the lowest mean unit cost rate since it has the longest system runtimes without failures. The age-based policy without parameter update always schedules maintenance for $\tau_*^{(0)} = 1.6$, and as the system does not fail before this time in any of the simulations, the system runtime is always 1.6 and the unit cost rate is $0.2 / 1.6 = 0.125$.

\begin{figure}
\includegraphics[width=\textwidth]{br1sim1fig5Tt01ac}
\caption{***Simulation study, each repetition consists of simulating 5 subsequent operational cycles,
where failure times are simulated according to prior parameters.}
\label{fig:br1sim1fig5Tt01}
\end{figure}

Case 2: failures earlier than expected,
failure times simulated using
$\lambda_k = 0.5 \ykz$ (Figure~\ref{fig:br1sim2fig5Tt01})

\begin{figure}
\includegraphics[width=\textwidth]{br1sim2fig5Tt01ac}
\caption{***Simulation study, each repetition consists of simulating 5 subsequent operational cycles,
where failure times are simulated as being earlier ($0.5 \times$ MTTF) than expected.}
\label{fig:br1sim2fig5Tt01}
\end{figure}

Case 3: failures later than expected,
failure times simulated using $\lambda_k = 2 \ykz$ from Table~\ref{tab:priorparams}.

\begin{figure}
\includegraphics[width=\textwidth]{br1sim3fig5Tt01ac}
\caption{***Simulation study, each repetition consists of simulating 5 subsequent operational cycles,
where failure times are simulated as being later ($2 \times$ MTTF) than expected.}
