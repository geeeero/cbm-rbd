...
only assumes that $\Rsysnow(t)$ can be calculated,
...
A preventive maintenance action is triggered as soon as $\tausnow$ falls below $\tsetup$,
the time needed to set up maintenance work.
...
which we will comment on in the Outlook (Section~\ref{sec:outlook}).
...
\begin{figure}
\includegraphics[width=\textwidth]{ghistfig1}
...
\begin{figure}
\includegraphics[width=\textwidth]{tauhistfig1}
\caption{$\tausnow$, $t^* = \tnow + \tausnow$ and $\gnow(\tausnow)$ as functions of $\tnow$
for the system depicted in Figure~\ref{fig:brakesys-layout},
assuming the failure history $P_2 = 3$, $P_3 = 4$, $C_2 = 6$, $C_3 = 7$, $H = 8$,
and $c_u = 1$, $c_p = 0.2$.
$\tausnow$ first decreases slightly, then remains basically constant during failure-free periods,
dropping at each component failure.
If $\tsetup = 0.5$, the drop of $\tausnow$ to $0.38$ at $\tnow = 8$ would trigger a maintenance action
at (absolute) time $8.5$. 
As $\tausnow$ does not change dramatically, $t^*$,
the cost-optimal moment of maintenance on the absolute timescale,
steadily increases, with small drops corresponding to $\tausnow$,
indicating that as time progresses, accumulated information indicates that maintenance can be postponed.
Note that $\gnow(\tausnow)$ is the momentary cost rate relating to the expected residual lifetime,
and not the overall cost rate.
