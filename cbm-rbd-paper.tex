...
To illustrate, for component with lifetimes measuring in days,
$\delta$ should be in the order of minutes;
...
Figure~\ref{fig:timeline} shows two example time lines for the system,
starting each with a new system (operational cycle) at t=0.
As described in Section~\ref{intro},
the health of the system is re-evaluated at 

two kinds of triggers 

health of system via $\tausnow$, which is described in detail in Section~***

In the left timeline example, 


***explain figure, triggers 1, 2, 3***
...
The horizontal bars depict, for subsequent times $\tnow$,
the time to the next economically optimal inspection $\tausnow$.
The times $\tnow$ for which $\tausnow$ is evaluated are
at regular intervals $\delta, 2\delta, 3\delta,\ldots$ (trigger 1),
and at the time of component failures (trigger 2),
marked by a circle on the time axis.
