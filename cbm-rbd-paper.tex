...
***This example was originally introduced in Ebeling (???).***
...
\draw[very thick] (gotonxt.west) -- ++(-3mm,0mm) -- ($(sysfail.west) + (-3mm,0mm)$);
...
\draw[pfeil] (update) -| (startup) node[midway, below right] {start new cycle};
\end{tikzpicture}
\caption{***operational procedure}
\label{fig:procedure}
\end{figure}
...
that a preventive maintenance action should be carried out, incurring cost $c_p$.
When the system fails, instead a corrective maintenance action is triggered, incurring cost $c_u$.
With both preventive and corrective maintenance the current cycle ends,
...
We assume that both preventive and corrective maintenance results in an (as good as) new system state by replacing all components,
...
that would merit a separate study.
...
c_u / (\tsysnow - \tnow) & \text{if } \tsysnow  <  \tnow + \tau \,.
\end{cases}
\label{eq:gtau}
\end{align}
...
\gnow(\tau) &= \E[g(\tau \mid \Tsysnow)] \nonumber\\ 
            &= \frac{c_p}{\tau} \Rsysnow(\tnow + \tau) + c_u \int_0^\tau \frac{1}{t} \fsysnow(\tnow + t) \dd t\,,
\label{eq:gtnowtau}
\end{align}
...
Also of interest is the total cost rate at $\tnow$, denoted by $\gtotalnow$.
...
where evaluations of $\fsysnow(\tnow + t)$ are calculated via differences of $\Rsysnow(\tnow + t)$ evaluations.
...
evaluations of $\Rsysnow(t)$ and $\fsysnow(t)$ only for $t \leq \tau$.
...
\section{Numerical examples for a complex system}
\label{sec:examples}
...
While it is quite unlikely that any type $C$ component will survive past time $6$,
...
We illustrate our procedure*** under three different failure time histories.
...
This is failure history used for Figures~\ref{fig:ghist1fig2} and \ref{fig:tauhist1fig2}.
Failures observed for type $C$ are thus considerably later than expected,
...
In total, due to the late type $C$ failures, prior assumptions are somewhat pessimistic.
This is reflected in Figure~\ref{fig:tauhist1fig2},
where $\tausnow$ first decreases but then tends to increase
during failure-free periods, adjusting prior expectations since none of the type $C$ components fail.

***move text from caption to here?***

The effect of the continuous parameter update is illustrated in Figure~\ref{fig:tauhist1fig3}.
This compares the results for our approach (as seen already in Figure~\ref{fig:tauhist1fig2})
with an approach that does not update the component parameters,
while still accounting for the fact that components age, and that a failed component changes the system layout.
...
The second failure history contains the failure times
...
i.e., the Weibull scale parameters chosen as $\ykz$ in Table~\ref{tab:priorparams}.
