...
and follow the system until $\tnow = 2.5$ only.
...
Model with update suggests to mainatin a bit earlier,
since $\tausnow$ drops below $\delta = 0.1$ at $\tnow = ***$,
while this happens for the no update model at $\tnow = ***$.

However, these differences are very small in comparison to the gains offered by the continuous update
...
and we follow the system until $\tnow = 0.5$***0.75?.
...
unlike in other scenarios, overall trend for $\tstarnow$ constant because of heavy drops,
