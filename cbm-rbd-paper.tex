...
\label{eq:gtnowtau}
\end{align}
again incorporating all that is known at time $\tnow$.
...
(Our procedure is illustrated for a number of current times $\tnow$ in Figure~\ref{fig:ghistfig2}.)
The prospective unit cost rate $\cstarnow = \gnow(\tausnow)$ corresponding to $\tausnow$
thus is the lowest unit cost rate attainable for the remaining lifetime of the system,
...
such that one can be sure to find $\tausnow$.
We calculate $\gnow(\tau)$ on the same grid,
and determine $\tausnow$ as the minimum over this grid.

We approximate the integral in \eqref{eq:gtnowtau} numerically by a sum,
...
P & $1.5$ & $3$\rule{1.5ex}{0ex} & $ 6.1$ & $1$\rule{1ex}{0ex} \\
  \bottomrule
\end{tabular}
\caption{Prior parameters for the four component types as used in the numeric examples.}
