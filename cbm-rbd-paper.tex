...
However, instead of a determining a threshold for a degradation signal like in usual CBM policies,
...
and whenever $\tausnow$ decreases below $\tsetup$, the timespan needed to set up maintenance work
(often referred to as setup time),
a preventive maintenance action is initiated.

The optimal time to maintenance $\tausnow$ is determined based on $\Rsysnow(t)$
using the expected unit cost rate for the current operational cycle, see Section~\ref{sec:costrate} below.
...
A cycle ends when the system is either maintained preventively at $\tnow + \tausnow$ (on an absolute time scale),
or the system fails, which triggers a corrective maintenance action.
We assume that either maintenance action takes negligible time,
and that either action results in an (as good as) new system state by replacing all components,
such that the system begins a new cycle begins under identical conditions.

***info from finished cycle is added to component knowledge base,
thus included in $\nkz$ and $\ykz$ for the following cycle.

Therefore, at each present time $\tnow$
(at a frequency small enough to be negligible on the system lifetime scale),
the current residual life distribution $\Rsysnow(t)$ is recalculated,
and the corresponding $\tausnow$ determined.

***as $\Rsysnow(t)$ on discrete grid until horizon, optimize $\tausnow$ over same discrete grid.
if $\gnow(\tau)$ always decreasing, corrective maintenance strategy! so we can switch maintenance policy!
